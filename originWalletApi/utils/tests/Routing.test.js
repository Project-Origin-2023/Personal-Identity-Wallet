...
it('should return 200 with "Empty List" message if there are no credentials', async () => {
  ...
  expect(response2.status).toBe(200);
  expect(response2.body).toEqual({"data": {"list": []}, "description": "OIDC Request with success", "success": true});
});
...
it('should return 500 with "Token Missing" message if token is missing', async () => {
  ...
  expect(response.body).toEqual({ success: false, description: 'Authorization token not found' });
}
);
